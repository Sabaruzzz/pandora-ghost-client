...
void    ImGui_ImplOpenGL3_RenderDrawData(ImDrawData* draw_data)
{
    ...
    GLboolean last_enable_alpha_test = glIsEnabled(GL_ALPHA_TEST);
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
    ...
    if (last_enable_alpha_test) glEnable(GL_ALPHA_TEST); else glDisable(GL_ALPHA_TEST);
#ifdef IMGUI_IMPL_OPENGL_MAY_HAVE_PRIMITIVE_RESTART
...
extern bool g_pandoraContextLost;
...
void ImGui_ImplOpenGL3_DestroyFontsTexture()
{
    ...
    if (bd->FontTexture)
    {
        if (!g_pandoraContextLost) glDeleteTextures(1, &bd->FontTexture);
        io.Fonts->SetTexID(0);
        bd->FontTexture = 0;
    }
}
...
{
    ImGui_ImplOpenGL3_Data* bd = ImGui_ImplOpenGL3_GetBackendData();
    if (bd->VboHandle)      { if (!g_pandoraContextLost) glDeleteBuffers(1, &bd->VboHandle); bd->VboHandle = 0; }
    if (bd->ElementsHandle) { if (!g_pandoraContextLost) glDeleteBuffers(1, &bd->ElementsHandle); bd->ElementsHandle = 0; }
    if (bd->ShaderHandle)   { if (!g_pandoraContextLost) glDeleteProgram(bd->ShaderHandle); bd->ShaderHandle = 0; }
