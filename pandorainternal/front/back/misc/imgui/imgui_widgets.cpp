...
bool ImGui::Keybind(const char* label, int* key, int* mode, bool show_label)
{
    ...
    const int safe_key = ImClamp(*key, 0, (int)IM_ARRAYSIZE(keys) - 1);
    active_key += keys[safe_key];
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, g.ActiveId == id || hovered ? c::keybind::background_hov : c::keybind::background, ImGui::GetIO().DeltaTime * 6.f);
    ...
    if (it_anim->second.alpha >= 0.01f)
    {
        ...
        SetNextWindowSize(ImVec2(100, 103));
...
bool ImGui::tabs(bool selected, const char* ico, const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, selected ? c::tabs::background_active : hovered ? c::tabs::background_hov : c::tabs::background, g.IO.DeltaTime * 6.f);
    it_anim->second.border = ImLerp(it_anim->second.border, selected ? c::tabs::border_active : c::tabs::border_inactive, g.IO.DeltaTime * 6.f);
    ...
    it_anim->second.scale = ImLerp(it_anim->second.scale, (hovered || selected) ? 1.045f : 1.0f, g.IO.DeltaTime * 10.f);

    GetWindowDrawList()->AddRectFilled(bb.Min, bb.Max, GetColorU32(it_anim->second.background), c::tabs::rounding);
    GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(it_anim->second.border), c::tabs::rounding);

    const float textScale = it_anim->second.scale;
    const float labelFontSize = g.FontSize * textScale;
    const ImVec2 scaledLabel = g.Font->CalcTextSizeA(labelFontSize, FLT_MAX, 0.0f, label);
    GetWindowDrawList()->AddText(g.Font, labelFontSize,
        ImVec2(bb.Min.x + 45.0f, bb.Min.y + (bb.GetHeight() - scaledLabel.y) * .5f),
        GetColorU32(it_anim->second.text), label);
    if (font::default_icon) {
        const float iconFontSize = 16.0f * textScale;
        const ImVec2 scaledIcon = font::default_icon->CalcTextSizeA(iconFontSize, FLT_MAX, 0.0f, ico);
        GetWindowDrawList()->AddText(font::default_icon, iconFontSize,
            ImVec2(bb.Min.x + 15.0f, bb.Min.y + (bb.GetHeight() - scaledIcon.y) * .5f),
            GetColorU32(it_anim->second.text), ico);
    }
...
struct button_state {
    ImVec4 background, border0, border1;
};
...
bool ImGui::config_selector(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    const std::string select_0(label ? label : "");
    const std::string select_1(select_0);
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, hovered ? c::button::background_hov : c::button::background, g.IO.DeltaTime * 6.f);
    it_anim->second.border0 = ImLerp(it_anim->second.border0, hovered || label == select_0 || label == select_1 ? c::accent : c::button::border, g.IO.DeltaTime * 6.f);
    it_anim->second.border1 = ImLerp(it_anim->second.border1, hovered || label == select_0 || label == select_1 ? c::accent_gradient : c::button::border, g.IO.DeltaTime * 6.f);

  //  it_anim->second.icon = ImLerp(it_anim->second.icon, label == select_0 || label == select_1 ? colors::checkbox::background_active : colors::button::config_ico, g.IO.DeltaTime * 9.f);

    GetWindowDrawList()->AddRectFilledMultiColor(rect.Min, rect.Max, GetColorU32(it_anim->second.border1), GetColorU32(it_anim->second.border0), GetColorU32(it_anim->second.border0), GetColorU32(it_anim->second.border1), c::button::rounding);

    GetWindowDrawList()->AddRectFilled(rect.Min + ImVec2(1, 1), rect.Max - ImVec2(1, 1), GetColorU32(it_anim->second.background), c::button::rounding);
  ...
  //  PushFont(fonts::icon);
  //  RenderTextClipped(rect.Min - ImVec2(12, -4), rect.Max - ImVec2(12, -4), label == select_1 ? "B" : "C", NULL, NULL, ImVec2(1.0f, 0.5f));
  //  PopFont();
  //  PopStyleColor();
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, hovered ? c::button::background_hov : c::button::background, g.IO.DeltaTime * 6.f);
    it_anim->second.border0 = ImLerp(it_anim->second.border0, IsItemActive() ? c::accent : c::button::border, g.IO.DeltaTime * 6.f);
    it_anim->second.border1 = ImLerp(it_anim->second.border1, IsItemActive() ? c::accent_gradient : c::button::border, g.IO.DeltaTime * 6.f);

    GetWindowDrawList()->AddRectFilledMultiColor(bb.Min, bb.Max, GetColorU32(it_anim->second.border1), GetColorU32(it_anim->second.border0), GetColorU32(it_anim->second.border0), GetColorU32(it_anim->second.border1), c::button::rounding);

    GetWindowDrawList()->AddRectFilled(bb.Min + ImVec2(1, 1), bb.Max - ImVec2(1, 1), GetColorU32(it_anim->second.background), c::button::rounding);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, ImS64* p_scroll_v, ImS64 size_avail_v, ImS64 size_contents_v, ImDrawFlags flags)
{
    ...
    window->DrawList->AddRect(grab_rect.Min, grab_rect.Max, GetColorU32(c::scrollbar::border), c::scrollbar::rounding);
...
bool ImGui::CustomImage(const char* id, ImTextureID texture_id, const ImVec2& size, const ImVec2& uv0, const ImVec2& uv1, ImU32 tint_col, ImGuiButtonFlags flags)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems) return false;
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(bb, window->GetID(id), &hovered, &held, flags);

    static std::map<ImGuiID, image_state> anim;
    auto it_anim = anim.find(window->GetID(id));

    if (it_anim == anim.end())
    {
        anim.insert({ window->GetID(id), image_state() });
        it_anim = anim.find(window->GetID(id));
    }

    it_anim->second.rotation = ImLerp(it_anim->second.rotation, -1.f, g.IO.DeltaTime * 12.f);

    ItemSize(bb);
    if (!ItemAdd(bb, window->GetID(id))) return false;

    ImRotateStart();
    window->DrawList->AddImage(texture_id, bb.Min + padding, bb.Max - padding, uv0, uv1, tint_col);
    ImRotateEnd(1.57f * it_anim->second.rotation);
...
struct check_state {
    ImVec4 background0, background1, text, mark;
};
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    bool hovered, held, pressed = ButtonBehavior(check_bb, id, &hovered, &held);
    if (pressed)
    {
        *v = !(*v);
        MarkItemEdited(id);
    ...
    it_anim->second.background0 = ImLerp(it_anim->second.background0, *v ? c::accent : hovered ? c::checkbox::background : c::checkbox::background, g.IO.DeltaTime * 6.f);
    it_anim->second.background1 = ImLerp(it_anim->second.background1, *v ? c::accent_gradient : hovered ? c::checkbox::background : c::checkbox::background, g.IO.DeltaTime * 6.f);
    ...
    GetWindowDrawList()->AddRectFilledMultiColor(check_bb.Min, check_bb.Max, GetColorU32(it_anim->second.background1), GetColorU32(it_anim->second.background0), GetColorU32(it_anim->second.background0), GetColorU32(it_anim->second.background1), c::checkbox::rounding);

    GetWindowDrawList()->AddRect(check_bb.Min - ImVec2(1, 1), check_bb.Max + ImVec2(1, 1), GetColorU32(c::checkbox::border), c::checkbox::rounding);
...
void ImGui::CheckboxKeybind(const char* label, bool* v, int* key, int* mode)
{
    ...
    Checkbox(label, v);
    SetCursorPosY(GetCursorPosY() - (GetStyle().ItemSpacing.y * 2) - 9);
    Keybind(key1.c_str(), key, mode, false);
...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags)
{
    ...
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddRectFilled(bb.Min + ImVec2(GetStyle().WindowPadding.x, 0), bb.Max - ImVec2(GetStyle().WindowPadding.x, 0), GetColorU32(c::separator::border));
            if (g.LogEnabled)
...
bool ImGui::BeginCombo(const char* label, const char* preview_value, float val, bool multi, ImGuiComboFlags flags)
{
    ...
    const float w = (flags & ImGuiComboFlags_NoPreview) ? arrow_size : ImMax(40.0f, GetContentRegionAvail().x - 16.0f);
    ...
    if (val < 5)
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (val * 35) : 0.f, g.IO.DeltaTime * 12.f);
    else
    it_anim->second.combo_size = ImLerp(it_anim->second.combo_size, it_anim->second.opened_combo ? (5 * 35) : 0.f, g.IO.DeltaTime * 12.f);

    it_anim->second.arrow_roll = ImLerp(it_anim->second.arrow_roll, it_anim->second.opened_combo ? -1.f : 1.f, g.IO.DeltaTime * 12.f);
    it_anim->second.background = ImLerp(it_anim->second.background, hovered ? c::combo::background_hov : c::combo::background, g.IO.DeltaTime * 6.f);
    ...
    ImRotateStart();
    PushFont(font::default_icon);
    RenderTextClipped(bb.Min + ImVec2(w - 25, 9.5f), ImVec2(value_x2 + 20, bb.Max.y), "F", NULL, NULL);
    PopFont();
    ImRotateEnd(1.57f * it_anim->second.arrow_roll);

    RenderTextClipped(bb.Min + ImVec2(9, (35 / 2) - (CalcTextSize(label).y / 2) ), ImVec2(value_x2, bb.Max.y), preview_value, NULL, NULL);
    PopStyleColor();

    GetWindowDrawList()->AddRectFilledMultiColor(ImVec2(ImMax(bb.Min.x, bb.Max.x - 150.0f), bb.Min.y + 1.0f),
        bb.Max, GetColorU32(c::combo::shadow), GetColorU32(it_anim->second.background),
        GetColorU32(it_anim->second.background), GetColorU32(c::combo::shadow));
    GetWindowDrawList()->AddRect(bb.Min, bb.Max, GetColorU32(c::combo::border), c::combo::rounding);
    ...
    ImGui::SetNextWindowPos(ImVec2(bb.Min.x + 2, bb.Max.y + 3));
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImGuiStyle& style = g.Style;
    const ImGuiID id = window->GetID(label);
    const float w = ImMax(40.0f, GetContentRegionAvail().x - 16.0f);

    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2(0, label_size.y + 4), window->DC.CursorPos + ImVec2(w, label_size.y + 19.f));
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, IsItemActive() ? c::slider::background_hov : c::slider::background, g.IO.DeltaTime * 6.f);
    it_anim->second.circle_anim = ImLerp(it_anim->second.circle_anim, IsItemActive() ? 2.f : 4.5f, g.IO.DeltaTime * 6.f);

    it_anim->second.circle = ImLerp(it_anim->second.circle, IsItemActive() ? c::accent : c::slider::circle, g.IO.DeltaTime * 6.f);
    ...
    if (grab_bb.Max.x > grab_bb.Min.x) {
        ...
        it_anim->second.slow_anim = ImLerp(it_anim->second.slow_anim, grab_bb.Min.x - frame_bb.Min.x, g.IO.DeltaTime * 45.f);

        GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min + ImVec2(0, 5), ImVec2(it_anim->second.slow_anim + frame_bb.Min.x, grab_bb.Max.y - 3), GetColorU32(c::accent_gradient), GetColorU32(c::accent), GetColorU32(c::accent), GetColorU32(c::accent_gradient), 100.f);


        GetWindowDrawList()->AddRect(frame_bb.Min + ImVec2(0, 4), frame_bb.Max - ImVec2(0, 4), GetColorU32(c::slider::border), 100.f);

        const float circle_x = ImClamp(it_anim->second.slow_anim + frame_bb.Min.x,
            frame_bb.Min.x + 6.0f, frame_bb.Max.x - 6.0f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(circle_x, grab_bb.Max.y - 6), 8.f, GetColorU32(c::slider::circle), 100.f);
        GetWindowDrawList()->AddCircleFilled(ImVec2(circle_x, grab_bb.Max.y - 6), it_anim->second.circle_anim, GetColorU32(c::accent), 100.f);

    }

    PushStyleColor(ImGuiCol_Text, GetColorU32(it_anim->second.text));
    RenderTextClipped(frame_bb.Min - ImVec2(0, 24), frame_bb.Max - ImVec2(0, 19), value_buf, value_buf_end, NULL, ImVec2(1.0f, 1.0f));
    PopStyleColor();

    GetWindowDrawList()->AddText(ImVec2(frame_bb.Min.x, frame_bb.Min.y - 24), GetColorU32(it_anim->second.text), label);
...
void ImGui::Textfield(const char* hint, char* buf, int buf_size, ImGuiInputTextFlags flags)
{
    std::string name = "##1", label = hint + name;

    ImGui::InputTextWithHint(label.c_str(), hint, buf, buf_size, flags);
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiInputTextCallback callback, void* callback_user_data)
{
    ...
    const ImGuiID id = window->GetID(label);
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImVec2 frame_size = CalcItemSize(size_arg, GetContentRegionMax().x - style.WindowPadding.x, (is_multiline ? g.FontSize * 8.0f : label_size.y) + style.FramePadding.y * 2.0f); // Arbitrary default of 8 lines high for multi-line
    ...
    it_anim->second.background = ImLerp(it_anim->second.background, hovered ? c::input::background_hov : c::input::background, g.IO.DeltaTime * 6.f);
    ...
    if (!is_multiline)
    {
        GetWindowDrawList()->AddRectFilledMultiColor(frame_bb.Min, frame_bb.Max, GetColorU32(it_anim->second.border1), GetColorU32(it_anim->second.border0), GetColorU32(it_anim->second.border0), GetColorU32(it_anim->second.border1), c::input::rounding);
    ...
    ImVec2 draw_pos = is_multiline ? draw_window->DC.CursorPos : frame_bb.Min + ImVec2(10, (label_size.y / 2) - (CalcTextSize(hint).y / 2) / 2 + 1 );
    ...
    if (render_cursor || render_selection)
    {
        ...
        if (render_selection)
        {
            ...
            ImU32 bg_color = GetColorU32(c::text::text_selected); // FIXME: current code flow mandate that render_cursor is always true here, we are leaving the transparent one for tests.
        ...
        if (render_cursor)
        {
            ...
            if (cursor_is_visible && cursor_screen_rect.Overlaps(clip_rect))
                draw_window->DrawList->AddLine(total_bb.Min + ImVec2(it_anim->second.slow, 6), total_bb.GetBL() - ImVec2(-it_anim->second.slow, 6), GetColorU32(c::text::text_active));
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags)
{
    ...
    if (!(flags & ImGuiColorEditFlags_NoSmallPreview))
    {
        ...
        if (BeginPopup("picker"))
        {
            if (g.CurrentWindow->BeginCount == 1)
            {
                ...
                ImGui::Button(buf, ImVec2((GetContentRegionMax().x - style.WindowPadding.x), 30));
    ...
    if (label != label_display_end && !(flags & ImGuiColorEditFlags_NoLabel))
    {
        ...
        PushStyleColor(ImGuiCol_Text, GetColorU32(c::text::text));
        ImGui::RenderTextClipped(window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 220, 0), window->DC.CursorPos + ImVec2(GetWindowContentRegionMax().x - 55, 22), buf, NULL, NULL, ImVec2(1.0f, 0.5f));
        PopStyleColor();
...
bool ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags, const float* ref_col)
{
    ...
    float bar0_pos_x = GetWindowPos().x + 20;
    ...
    GetForegroundDrawList()->AddRectFilledMultiColorRounded(ImVec2(bar0_pos_x, bar_pos.y + 4), ImVec2(bar0_pos_x + bars_width, bar_pos.y + sv_bar_size - 5), GetColorU32(c::picker::background), 0, 0, 0, 0, c::picker::rounding, ImDrawCornerFlags_All);
    ...
    if (alpha_bar)
    {
        ...
        GetForegroundDrawList()->AddRectFilledMultiColorRounded(bar1_bb.Min + ImVec2(0, 3), bar1_bb.Max - ImVec2(0, 10), GetColorU32(c::picker::background), 0, 0, 0, 0, c::picker::rounding, ImDrawCornerFlags_All);
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    it_anim->second.line0 = ImLerp(it_anim->second.line0, selected ? c::accent : c::combo::border, g.IO.DeltaTime * 6.f);
    it_anim->second.line1 = ImLerp(it_anim->second.line1, selected ? c::accent_gradient : c::combo::border, g.IO.DeltaTime * 6.f);

    it_anim->second.line_offset = ImLerp(it_anim->second.line_offset, selected ? 10.f : 00.f, g.IO.DeltaTime * 6.f);

    ImGui::GetWindowDrawList()->AddRectFilledMultiColor(rect.Min + ImVec2(0, 15 - it_anim->second.line_offset), rect.Max - ImVec2(-3, 5 - it_anim->second.line_offset), GetColorU32(it_anim->second.line1), GetColorU32(it_anim->second.line1), GetColorU32(it_anim->second.line0), GetColorU32(it_anim->second.line0), 100.f, ImDrawCornerFlags_Right);
...
bool ImGui::BeginListBox(const char* label, const ImVec2& size_arg)
{
    ...
    ImVec2 size = ImFloor(CalcItemSize(size_arg, CalcItemWidth(), GetTextLineHeightWithSpacing() * 7.25f));
